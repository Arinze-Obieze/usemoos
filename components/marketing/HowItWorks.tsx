...
export default function HowItWorks() {
  return (
    <section className={cx(styles, "sec")} id="how">
      <div className={cx(styles, "wrap")}>
        <div className={cx(styles, "sec-head")}>
          <div className={cx(styles, "sec-tag")}>Onboarding</div>
          <h2 className={cx(styles, "sec-title")}>
            From scattered to searchable in <em>an afternoon</em>.
          </h2>
        </div>
        <div className={cx(styles, "how-grid")}>
          {steps.map((s) => (
            <div key={s.num} className={cx(styles, "how-step")}>
              <div className={cx(styles, "how-num")}>{s.num}</div>
              <h4>{s.heading}</h4>
              <p>{s.body}</p>
            </div>
          ))}
